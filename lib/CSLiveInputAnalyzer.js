...
(function () {
  ...
  this.enable_hold_to_generate = function () {
    ...
    this.generativeClip.generate_and_append_async(4 * 4, function () {
      ...
      me.clipPlayingWatcher = new LiveAPI(function () {

        var playingPosition = this.get("playing_position")[0];

        // if we're near the end of the generated part, need to generate more
        if (me.generativeClip._currentEndTime - playingPosition < 8) {
          me.generativeClip.generate_and_append_async(4 * 4, function () {
      ...
      }, me.clip.path.slice(1, -1));
      me.clipPlayingWatcher.property = "playing_position";
