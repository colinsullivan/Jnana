...
(function () {
   ...
   *  material from statistical analysis of the live input.
   **/
  this.genClips = [];
  ...
  this.init = function () {
    ...
    for (i = 0; i < clipSlots.length; i += 2) {
      ...
      if (clip.id !== "0" && clipName.match(/-gen[\d]*$/)) {
    ...
    this.inputAnalyzer = new CS.InputAnalyzer({
      phrase_finished_callback: function (phrase) {
        var roundedPhraseDuration,
          // use first clip
          generativeClip = me.genClips[0],
          previousShouldAutoTrain;

        generativeClip._phraseGenerator.incorporate_phrase(phrase);
        ...
        if (me.shouldAutoRespond) {
          generativeClip.generate_and_append_async(
            ...
            function () {
              ...
              generativeClip.set_playbackEndedCallback(function () {
                me.shouldAutoTrain = previousShouldAutoTrain;
                generativeClip.set_playbackEndedCallback(null);
                // no more midi passthrough
                me.set_midi_passthrough(false);
              });
              // play clip
              generativeClip._clip.call("fire");
            }
          );
        }
      }
  ...
  this.enable_hold_to_generate = function () {
    ...
    // generate initial phrase in all clips
    for (i = 0; i < genClips.length; i++) {
      genClip = genClips[i];
      genClip.generate_and_append(4 * 4);
  ...
  this.disable_hold_to_generate = function () {
    ...
    this.trackStoppingWatcher = new LiveAPI(function () {
      ...
      if (this.get("playing_slot_index")[0] === -1) {

        me.shouldAutoTrain = me.prevShouldAutoTrain;
        me.set_midi_passthrough(false);
        me.trackStoppingWatcher = null;
      }
