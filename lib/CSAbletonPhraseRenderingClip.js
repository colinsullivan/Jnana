...
(function () {
  ...
  CS.Ableton.PhraseRenderingClip.prototype = {
     ...
     *
     *  @param  CS.Phrase   phrase    The phrase to insert.
     *  @param  Number      tStart    The time at which to insert the phrase.
     **/
    insert_phrase: function (phrase, tStart) {
     ...
     *  at initialization), then insert it at a particular point in time
     *  on the clip.
     *
     *  @param  Number    tStart    Point in time to insert generated phrase
     *  @param  Number    duration  Duration of phrase to generate
     **/
    generate_and_insert: function (tStart, duration) {
      var generatedPhrase;

      this._isGenerating = true;

      generatedPhrase = this._phraseGenerator.generate_phrase(duration);
      this.insert_phrase(generatedPhrase, tStart);
     ...
     **/
    generate_and_insert_async: function (tStart, duration, callback) {
      var genTask;
      ...
      genTask = new Task(function (args) {
        this.generate_and_insert(args[0], args[1]);
        args[3]();
    ...
    _generate_and_append_loop: function () {
      ...
      this.generate_and_insert(tStart, loopLength);
  ...
  CS.Ableton.SelfGeneratingClip.prototype._generate_and_append_loop = function () {
    CS.Ableton.PhraseRenderingClip.prototype._generate_and_append_loop.apply(this, arguments);
    this._playsSinceLastGenerate = 0;
  };
