...
(function () {
  ...
  CS.MarkovTable = function (params) {
    ...
    /**
     *  Mapping of previous state keys and probabilities associated with
     *  those previous states being the first in a "traversal" through
     *  the system.
     *
     *  i.e. for a second order system,
     *      
     *      this._startingStateProbabilities["60->62"] = 0.5;
     *
     *  if there is a 50% chance that 60->62 will be the starting state's
     *  "previous" path.
     **/
    this._startingStateProbabilities = null;
  ...
  CS.MarkovTable.prototype = {
    ...
    clear: function () {
      ...
      this._destStates = [];
      this._prevStates = [];
      this._startingStateProbabilities = {};
    },
    /**
     *  Get next state.
     **/
    next: function () {
      ...
      // (this will only happen on first run)
        ...
        if (this._rowsList.length === 0) {
          throw new Error("Attempting to call next on an empty table.");
        }
        ...
        this._prevStates = row._prevStates;
      }
    ...
    _get_or_create_row_from_transition: function (transitionData) {
      ...
      if (typeof row === "undefined" || row === null) {
        ...
        // this row currently has no chance of being the starting state
        this._startingStateProbabilities[key] = 0.0;
      }
